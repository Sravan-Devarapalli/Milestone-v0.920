...
AS 
    BEGIN
        ...
        FROM    @MinStartdates minSD
                INNER JOIN dbo.PersonStatusHistory PSH ON PSH.PersonId = minSD.personId
                                                          AND minSD.minStartDate = PSH.StartDate
        ...
        WHERE   p.IsStrawman = 0
                ...
                AND ( ( @StartDate < ISNULL(PSH.EndDate, dbo.GetFutureDate())
                        AND @EndDate > PSH.StartDate
                      )
                      OR ( minSD.statusId = 1
                           AND P.HireDate < minSD.minStartDate
                           AND @StartDate < minSD.minStartDate
