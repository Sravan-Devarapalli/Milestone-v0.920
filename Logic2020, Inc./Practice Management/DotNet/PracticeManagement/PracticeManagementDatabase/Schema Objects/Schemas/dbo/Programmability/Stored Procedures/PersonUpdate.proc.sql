...
AS
	...
	BEGIN
		...
		BEGIN
			...
			JOIN TimeEntry TE ON TE.TimeEntryId = TEH.TimeEntryId AND TE.PersonId = @PersonId AND TE.ChargeCodeDate > @TerminationDate AND TE.IsAutoGenerated = 1

			DELETE TE
			FROM TimeEntry TE
			WHERE TE.PersonId = @PersonId AND TE.ChargeCodeDate > @TerminationDate AND TE.IsAutoGenerated = 1		

			--Delete all the Recursive records later @TerminationDate
		...
		BEGIN		
			...
			DELETE TEH
			FROM TimeEntry TE
			JOIN TimeEntryHours TEH ON TEH.TimeEntryId = TE.TimeEntryId AND TE.PersonId = @PersonId 
										AND TE.ChargeCodeDate > @TerminationDate AND TE.ChargeCodeId IN (@PTOChargeCodeId, @HolidayChargeCodeId)
			
			DELETE TE
			FROM TimeEntry TE
			WHERE TE.PersonId = @PersonId AND TE.ChargeCodeDate > @TerminationDate AND TE.ChargeCodeId IN (@PTOChargeCodeId, @HolidayChargeCodeId)
		...
		BEGIN
			...
			LEFT JOIN TimeEntry TE ON TE.PersonId = p.Person AND TE.ChargeCodeDate = C.Date AND TE.ChargeCodeId IN (@HolidayChargeCodeId, @PTOChargeCodeId)
			...
			SELECT TE.TimeEntryId,
					...
					@PersonUserId,
					0, --NON billable.
					CASE WHEN PC.PersonId IS NOT NULL AND PC.DayOff = 1 AND ISNULL(PC.IsFloatingHoliday, 0) = 0 AND PC.IsFromTimeEntry <> 1 THEN 2 ELSE 1 END
			FROM Calendar C
			JOIN Pay p ON C.Date BETWEEN p.StartDate and ( CASE WHEN @TerminationDate IS NOT NULL AND @TerminationDate < p.EndDate - 1 THEN @TerminationDate
																ELSE p.EndDate- 1 END) AND p.Person = @PersonId AND p.Timescale = @W2SalaryId
			JOIN TimeEntry TE ON TE.PersonId = p.Person AND TE.ChargeCodeDate = C.Date AND TE.ChargeCodeId IN (@HolidayChargeCodeId, @PTOChargeCodeId)
