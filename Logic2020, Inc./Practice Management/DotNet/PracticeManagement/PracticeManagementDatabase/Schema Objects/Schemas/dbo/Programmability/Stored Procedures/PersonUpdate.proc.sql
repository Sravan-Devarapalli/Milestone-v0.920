	...
	BEGIN
		...
		BEGIN
			...
			SELECT @CurrentPMTime AS EntryDate
				...
				,mpe.HoursPerDay
				,CASE WHEN PC.PersonId IS NOT NULL AND PC.DayOff = 1 AND ISNULL(PC.IsFloatingHoliday,0) = 0 THEN @PTOTimeTypeId ELSE @HolidayTimeTypeId END
				,@PersonUserId
				,CASE WHEN PC.PersonId IS NOT NULL AND PC.DayOff = 1 AND ISNULL(PC.IsFloatingHoliday,0) = 0 THEN 'PTO'
						...
						ELSE ISNULL(crh.HolidayDescription,'') END
				,m.IsChargeable
				,1
			...
			LEFT JOIN TimeEntries te ON te.MilestonePersonId = mpe.MilestonePersonId AND te.MilestoneDate = crh.Date AND te.TimeTypeId  IN (@HolidayTimeTypeId , @PTOTimeTypeId)
			LEFT JOIN PersonCalendar PC ON PC.Date = crh.Date AND PC.PersonId = p.Person AND PC.DayOff = 1
			WHERE ((crh.DayOff = 1  AND DATEPART(DW,crh.DATE) NOT IN (1,7) )
