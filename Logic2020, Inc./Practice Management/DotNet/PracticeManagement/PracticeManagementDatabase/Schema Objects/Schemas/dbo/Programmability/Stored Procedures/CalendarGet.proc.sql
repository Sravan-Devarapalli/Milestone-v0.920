...
AS 
    ...
    SELECT  cal.Date ,
            ...
            CAST(CASE WHEN pcal.Date IS NULL THEN 1
                      ELSE 0
            ...
            ( CASE WHEN cal.DayOff = 1 THEN cal.HolidayDescription
                   WHEN pcal.DayOff = 1
                        AND pcal.TimeTypeId IN ( @HolidayTimeTypeId,
                                                 @ORTTimeTypeId )
                   THEN pcal.Description + dbo.GetApprovedByName(cal.Date,
                                                              pcal.TimeTypeId,
                                                              @PersonId)
    ...
    FROM    dbo.Calendar AS cal
            ...
            LEFT JOIN dbo.MilestonePerson MP ON MP.PersonId = pcal.PersonId
                                                AND MP.MilestoneId = @DefaultMilestone
    ...
    SELECT  cal.Date ,
            ...
            CAST(CASE WHEN pcal.Date IS NULL
                           OR CONVERT(DATE, pcal.Date) < CONVERT(DATE, dbo.GettingPMTime(GETUTCDATE()))
                      THEN 1
                      ELSE 0
                 END AS BIT) AS [ReadOnly] ,
            cal.IsRecurring ,
