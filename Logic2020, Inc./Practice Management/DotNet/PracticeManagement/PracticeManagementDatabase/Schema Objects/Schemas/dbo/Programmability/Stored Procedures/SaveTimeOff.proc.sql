...
BEGIN
	...
	*/
	...
	BEGIN
		...
		*/
		...
		(
			...
			LEFT JOIN PersonCalendar AD ON PC.PersonId = AD.PersonId AND AD.DayOff = 1 AND AD.IsSeries = 1 AND PC.TimeTypeId = AD.TimeTypeId AND PC.ActualHours = AD.ActualHours AND ISNULL(PC.ApprovedBy, 0) = ISNULL(AD.ApprovedBy, 0)
			...
			LEFT JOIN PersonCalendar EDFS ON AD.PersonId = EDFS.PersonId AND EDFS.DayOff = 1 AND EDFS.IsSeries = 1 AND EDFS.TimeTypeId = AD.TimeTypeId AND EDFS.ActualHours = AD.ActualHours AND ISNULL(EDFS.ApprovedBy, 0) = ISNULL(AD.ApprovedBy, 0)
		...
		JOIN ExcludeDays ED ON ED.PersonId = PC.PersonId AND ED.Date = PC.Date
		...
		SELECT @PersonId, DEH.Date, @DayOff, @TimeTypeId, @ActualHours, @Description, @IsSeries, 0, CASE WHEN @TimeTypeId = @ORTTimeTypeId THEN @ApprovedBy ELSE NULL END
		...
		FROM PersonCalendar PC
		JOIN @DaysExceptHolidays DEH ON PC.PersonId = @PersonId AND PC.Date = DEH.Date AND (PC.TimeTypeId <> @TimeTypeId OR PC.ActualHours <> @ActualHours OR ISNULL(PC.ApprovedBy, 0) <> ISNULL(@ApprovedBy, 0))
		...
		(
			...
			LEFT JOIN PersonCalendar CD ON CD.PersonId = PC.PersonId AND CD.IsSeries = 1 AND PC.TimeTypeId = CD.TimeTypeId AND PC.ActualHours = CD.ActualHours AND ISNULL(PC.ApprovedBy, 0) = ISNULL(CD.ApprovedBy, 0)
		...
		UPDATE PC
			SET IsSeries = ED.IsSeries
		FROM PersonCalendar PC
		JOIN ExcludeDates ED ON ED.PersonId = PC.PersonId AND ED.Date = PC.Date
	...
	BEGIN
		...
		(
			SELECT  PC.PersonId 'PersonId', APC.Date 'Date', CONVERT(BIT, 0) 'IsSeries'
			FROM Calendar C
			JOIN PersonCalendar PC ON C.Date IN (@StartDate, @EndDate) AND PersonId = @PersonId AND C.Date = PC.Date AND PC.DayOff = 1 AND PC.IsSeries = 1
			LEFT JOIN PersonCalendar APC ON PC.PersonId = APC.PersonId AND PC.DayOff = 1 AND APC.IsSeries = 1 AND APC.TimeTypeId = PC.TimeTypeId AND APC.ActualHours = PC.ActualHours AND ISNULL(APC.ApprovedBy, 0) = ISNULL(PC.ApprovedBy, 0)
			...
			LEFT JOIN PersonCalendar AFAPC ON APC.PersonId = AFAPC.PersonId AND AFAPC.DayOff = 1 AND AFAPC.IsSeries = 1 AND AFAPC.TimeTypeId =APC.TimeTypeId AND AFAPC.ActualHours = APC.ActualHours AND ISNULL(APC.ApprovedBy, 0) = ISNULL(AFAPC.ApprovedBy, 0)
		...
		WHERE PersonId = @PersonId AND Date BETWEEN @StartDate AND @EndDate
	END
	...
	INNER JOIN dbo.ChargeCode CC ON TE.ChargeCodeDate BETWEEN @StartDate AND @EndDate AND TE.PersonId = @PersonId AND TE.ChargeCodeId = CC.Id
	INNER JOIN dbo.TimeType TT ON TT.TimeTypeId = CC.TimeTypeId AND TT.IsAdministrative = 1 AND TT.TimeTypeId <> @HolidayTimeTypeId
	INNER JOIN dbo.TimeEntryHours TEH ON TEH.TimeEntryId = TE.TimeEntryId
	INNER JOIN dbo.Calendar C ON C.Date = TE.ChargeCodeDate AND C.DayOff = 0
	LEFT JOIN dbo.PersonCalendar PC ON PC.Date = TE.ChargeCodeDate AND PC.TimeTypeId = CC.TimeTypeId AND PC.PersonId = TE.PersonId AND PC.DayOff = 1
	WHERE PC.Date IS NULL
	...
	INNER JOIN dbo.ChargeCode CC ON PC.Date BETWEEN @StartDate AND @EndDate AND PC.PersonId = @PersonId AND PC.DayOff = 1 AND PC.TimeTypeId = CC.TimeTypeId
	INNER JOIN dbo.TimeEntry TE ON TE.PersonId = PC.PersonId AND TE.ChargeCodeDate = PC.Date
	INNER JOIN dbo.ChargeCode TECC ON TE.ChargeCodeId = TECC.Id
	INNER JOIN dbo.TimeType TT ON TT.TimeTypeId = TECC.Id AND TT.IsAdministrative = 1 AND (TE.ChargeCodeId <> CC.Id OR TE.Note <> PC.Description)
	...
	INNER JOIN dbo.TimeEntryHours TEH ON TE.TimeEntryId = TEH.TimeEntryId AND PC.ActualHours <> TEH.ActualHours
	...
	FROM dbo.PersonCalendar PC
	INNER JOIN dbo.Calendar C ON PC.Date BETWEEN @StartDate AND @EndDate AND PC.PersonId = @PersonId AND PC.DayOff = 1 AND C.Date = PC.Date AND C.DayOff <> 1
	INNER JOIN dbo.Person P ON P.PersonId = PC.PersonId AND P.IsStrawman = 0
	INNER JOIN dbo.Pay pay ON p.PersonId = pay.Person AND pay.Timescale = 2
	...
	SELECT TE.TimeEntryId,
			...
			@ModifiedBy,
			0,--Non Billable
			CASE WHEN PC.IsFromTimeEntry <> 1 THEN 2 ELSE 1 END --Inserting timeEntries with Approved Status.
	FROM dbo.PersonCalendar PC
	INNER JOIN dbo.Calendar C ON PC.Date BETWEEN @StartDate AND @EndDate AND PC.PersonId = @PersonId AND PC.DayOff = 1 AND C.Date = PC.Date AND C.DayOff <> 1
	INNER JOIN dbo.Person P ON P.PersonId = PC.PersonId AND P.IsStrawman = 0
	INNER JOIN dbo.Pay pay ON p.PersonId = pay.Person AND pay.Timescale = 2
