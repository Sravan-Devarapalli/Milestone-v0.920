...
BEGIN
	...
	*/
	...
	INNER JOIN dbo.TimeEntry TE ON TEH.TimeEntryId = TE.TimeEntryId	AND TE.PersonId = @PersonId
    INNER JOIN dbo.PersonCalendar PC ON PC.PersonId=@PersonId
	WHERE  (TE.ChargeCodeDate > @TerminationDate OR (PC.SubstituteDate=TE.ChargeCodeDate AND PC.DayOff=0 AND TE.ChargeCodeId=2))

	DELETE TE
	FROM dbo.TimeEntry TE 
	INNER JOIN dbo.PersonCalendar PC ON PC.PersonId=@PersonId 
	WHERE  TE.PersonId = @PersonId
			AND (TE.ChargeCodeDate > @TerminationDate OR (PC.SubstituteDate=TE.ChargeCodeDate AND PC.DayOff=0 AND TE.ChargeCodeId=2))
   

   
   ----FOR Getting Substitute dates when the termination date is below the holiday date and termination date is above the substitute date  
   
   DECLARE @DeleteSubsDates1 TABLE ([date] DATETIME)

   INSERT INTO @DeleteSubsDates1
   SELECT PC.SubstituteDate
   FROM PersonCalendar PC 
   WHERE PC.PersonId=@PersonId AND PC.Date > @TerminationDate AND PC.SubstituteDate IS NOT NULL AND PC.SubstituteDate< @TerminationDate AND PC.DayOff=0 

   DELETE PC
   FROM dbo.PersonCalendar PC 
   LEFT JOIN @DeleteSubsDates1 SUB ON PC.PersonId=@PersonId AND SUB.date=PC.Date 
   WHERE PC.Date > @TerminationDate OR SUB.date=PC.Date
   
   DECLARE @SubsAfterTermination BIT=0

   SELECT @SubsAfterTermination=1
   FROM dbo.PersonCalendar PC
   WHERE PC.PersonId=@PersonId AND PC.Date < @TerminationDate AND PC.SubstituteDate IS NOT NULL AND PC.SubstituteDate > @TerminationDate AND PC.DayOff=0 

   --UPDATING PERSONCALANDER REVERSE CASE AS DAYOFF=1 IF TERMINATION DATE IS ABOVE THE ACTUAL HOLIDAY AND BELOW THE SUBSTITUTE DATE
   IF @SubsAfterTermination=1
   BEGIN

   DECLARE @DeleteSubsDates2 TABLE ([date] DATETIME)

   INSERT INTO @DeleteSubsDates2
   SELECT PC.Date
   FROM PersonCalendar PC 
   WHERE PC.PersonId=@PersonId AND PC.Date < @TerminationDate AND PC.SubstituteDate IS NOT NULL AND PC.SubstituteDate > @TerminationDate AND PC.DayOff=0 
	   ...
	   INSERT  INTO [dbo].[TimeEntry]
		...
		SELECT @PersonId,2,CAL.Date,CAL.HolidayDescription,0,1,1
		FROM dbo.PersonCalendar PC
		INNER JOIN dbo.Calendar CAL ON PC.PersonId=@PersonId AND PC.Date=CAL.Date
		WHERE PC.Date < @TerminationDate AND PC.SubstituteDate > @TerminationDate AND PC.DayOff=0
		...
		SELECT TE.TimeEntryId,8,@CurrentPMTime,@CurrentPMTime,@ModifiedBy,0,2
		FROM dbo.PersonCalendar PC
		INNER JOIN dbo.Calendar CAL ON PC.PersonId=@PersonId AND PC.Date=CAL.Date
		INNER JOIN DBO.TimeEntry TE ON TE.PersonId=@PersonId AND TE.ChargeCodeId=2 AND TE.ChargeCodeDate=CAL.Date
		WHERE PC.Date < @TerminationDate AND PC.SubstituteDate > @TerminationDate AND PC.DayOff=0

   DELETE PC
   FROM dbo.PersonCalendar PC 
   LEFT JOIN @DeleteSubsDates2 SUB ON PC.PersonId=@PersonId AND SUB.date=PC.Date 
   WHERE PC.Date > @TerminationDate OR SUB.date=PC.Date
