...
AS
	...
	*/
	...
	JOIN @Dates rhd ON rhd.[date] BETWEEN pay.StartDate AND (CASE WHEN p.TerminationDate IS NOT NULL AND pay.EndDate - 1 > p.TerminationDate THEN p.TerminationDate
																	ELSE pay.EndDate - 1
																	END)
	JOIN TimeEntry TE ON TE.PersonId = P.PersonId AND TE.ChargeCodeDate = rhd.date AND TE.ChargeCodeId IN (@HolidayChargeCodeId, @PTOChargeCodeId)
	...
	BEGIN
		...
		BEGIN
			...
			JOIN Pay pay ON pay.Person = PC.PersonId AND pay.Timescale = 2 AND d.date BETWEEN pay.StartDate AND (CASE WHEN p.TerminationDate IS NOT NULL AND pay.EndDate - 1 > p.TerminationDate THEN p.TerminationDate
																																ELSE pay.EndDate - 1
																																END)
			JOIN TimeEntry TE ON TE.PersonId = PC.PersonId AND TE.ChargeCodeDate = PC.Date
		...
		BEGIN
			...
			DELETE TEH
			FROM TimeEntry TE
			JOIN Person P ON TE.PersonId = P.PersonId AND P.PersonId = @PersonId AND P.IsStrawman = 0 AND TE.ChargeCodeId = @PTOChargeCodeId AND DATEPART(DW, @Date) NOT IN (1,7) AND TE.ChargeCodeDate = @Date
			JOIN TimeEntryHours TEH ON TEH.TimeEntryId = TE.TimeEntryId
	...
	BEGIN
		...
		JOIN @Dates rhd ON rhd.[date] BETWEEN pay.StartDate AND (CASE WHEN p.TerminationDate IS NOT NULL AND pay.EndDate - 1 > p.TerminationDate THEN p.TerminationDate
																															ELSE pay.EndDate - 1
																															END)
		LEFT JOIN TimeEntry TE ON TE.PersonId = P.PersonId AND TE.ChargeCodeDate = rhd.Date
		...
		JOIN @Dates rhd ON rhd.[date] BETWEEN pay.StartDate AND (CASE WHEN p.TerminationDate IS NOT NULL AND pay.EndDate - 1 > p.TerminationDate THEN p.TerminationDate
																															ELSE pay.EndDate - 1
																															END)
		JOIN TimeEntry TE ON TE.PersonId = P.PersonId AND TE.ChargeCodeDate = rhd.Date
									AND ((@PersonId IS NULL AND TE.ChargeCodeId = @HolidayChargeCodeId) 
											OR (@PersonId IS NOT NULL AND TE.ChargeCodeId = @PTOChargeCodeId)
										)
		JOIN Calendar c ON c.Date = rhd.date AND DATEPART(DW, c.date) NOT IN (1,7)
		...
		BEGIN
			...
			BEGIN
				...
				DELETE TEH
				FROM TimeEntry TE
				JOIN Person P ON P.PersonId = TE.PersonId AND P.PersonId = @PersonId AND P.IsStrawman = 0
				JOIN TimeEntryHours TEH ON TEH.TimeEntryId = TE.TimeEntryId
				WHERE DATEPART(DW, @Date) NOT IN (1,7) AND TE.ChargeCodeDate = @Date AND TE.ChargeCodeId = @PTOChargeCodeId
