...
BEGIN
	...
	DECLARE @Temp TABLE(PersonId INT,StartDate DATETIME,EndDate DATETIME,Hours REAL,HoursInRange REAL,GroupBy INT,IsGap BIT)
	...
	WHERE (@MilestonePersonId IS NULL OR MP.MilestonePersonId = @MilestonePersonId)
		...
		FOR
		 WITH MilestoneRecordsContinuity
			...
			SELECT PHD.PersonId,PHD.Date,PHD.Hours
		...
		@PersonIdC, @startDate,@SummedHours
		...
		@NextPersonId,@stopDate,@Hours
		 ...
		 BEGIN
			 ...
			 BEGIN
				...
				INSERT INTO @Temp
				SELECT @PersonIdC,@startDate,@beforestopDate,@SummedHours,@HoursInRange,@Groupby,@IsGap
			 ...
			 BEGIN
				 ...
				 IF(@IsGap = 0) AND DATEDIFF(DAY,@beforestopDate,@stopDate) > 1
				 BEGIN
					SET @IsGap = 1
				 END
				...
				BEGIN
					...
					IF @SummedHours+@Hours >= 520 AND @HoursInRange+@Hours >= 240
					BEGIN
						SET @Groupby = @Groupby+1
					END
					INSERT INTO @Temp
					SELECT @PersonIdC,@TempStartDate,@stopDate,@SummedHours+@Hours,@HoursInRange+@Hours,@Groupby,@IsGap
					SET @HoursInRange = 0
					SET @IsGap = 0
				END
				SET @beforestopDate = @stopDate
		 ...
		 ELSE
		 BEGIN
			IF @SummedHours >= 520 AND @HoursInRange >= 240
			BEGIN
				 SET @Groupby = @Groupby+1
			END
			INSERT INTO @Temp
			SELECT @PersonIdC,@TempStartDate,@beforestopDate,@SummedHours,@HoursInRange,@Groupby,@IsGap
		...
		SELECT CASE WHEN @ProjectId IS NULL THEN @ProjectIdLocal ELSE @ProjectId END,
			   Max(PersonId),
			   MIN(StartDate),
			   MAX(EndDate), 
