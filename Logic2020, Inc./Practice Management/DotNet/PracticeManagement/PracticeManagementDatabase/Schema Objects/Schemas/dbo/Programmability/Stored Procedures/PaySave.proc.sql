...
AS
	...
	FROM CompanyRecurringHolidaysByPeriod(ISNULL(@PreviousRecordStartDate,@StartDate), ISNULL(@NextRecordEndDate,@EndDate)-1) crh
	JOIN Pay p ON crh.Date BETWEEN p.StartDate AND P.EndDate-1 AND p.Timescale = @W2SalaryId
	...
	AND crh.Date >= @Today
	AND (crh.Date <= @TerminationDate OR @TerminationDate IS NULL)

	DELETE TE 
	FROM dbo.TimeEntries TE
	JOIN CompanyRecurringHolidaysByPeriod(ISNULL(@PreviousRecordStartDate,CASE WHEN @StartDate > @OLD_StartDate  THEN @OLD_StartDate ELSE @StartDate END), 
							ISNULL(@NextRecordEndDate,CASE WHEN @EndDate < @OLD_EndDate THEN @OLD_EndDate ELSE @EndDate END)-1) crh
	ON TE.MilestoneDate = crh.Date AND TE.IsAutoGenerated = 1
	...
	LEFT JOIN Pay P ON P.Person = mp.PersonId AND crh.Date BETWEEN p.StartDate AND P.EndDate-1
	WHERE crh.Date >= @Today AND (P.Person IS NULL OR  p.Timescale <> @W2SalaryId)
