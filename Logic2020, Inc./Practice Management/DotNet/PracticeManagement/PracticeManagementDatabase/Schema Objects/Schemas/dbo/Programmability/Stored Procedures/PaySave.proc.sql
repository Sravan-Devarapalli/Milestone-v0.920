...
AS
	...
	JOIN dbo.Calendar C ON (C.Date BETWEEN ISNULL(@PreviousRecordStartDate,CASE WHEN @StartDate > @OLD_StartDate  THEN @OLD_StartDate ELSE @StartDate END) AND 
		...
		AND TE.ChargeCodeDate = C.Date AND TE.ChargeCodeId  IN (@HolidayChargeCodeId ) AND TE.PersonId = @PersonId
	...
	WHERE C.Date >= @Today AND (P.Person IS NULL OR  p.Timescale <> @W2SalaryId)
		...
		LEFT JOIN dbo.TimeEntry TE ON TE.PersonId = p.Person AND TE.ChargeCodeDate = C.Date AND TE.ChargeCodeId  IN (@HolidayChargeCodeId , @PTOChargeCodeId)
		...
		JOIN dbo.Person per ON per.PersonId = p.Person AND per.IsStrawman = 0
		LEFT JOIN dbo.PersonCalendar PC ON PC.Date = C.Date AND PC.PersonId = p.Person AND PC.DayOff = 1
		JOIN dbo.TimeEntry TE ON TE.PersonId = p.Person AND TE.ChargeCodeDate = C.Date AND TE.ChargeCodeId  IN (@HolidayChargeCodeId , @PTOChargeCodeId)
		LEFT JOIN dbo.TimeEntryHours TEH ON  TEH.TimeEntryId = TE.TimeEntryId						   
		WHERE (C.Date BETWEEN ISNULL(@PreviousRecordStartDate,@StartDate) AND ISNULL(@NextRecordEndDate,@EndDate)-1)
		AND ((C.DayOff = 1  AND DATEPART(DW,C.DATE) NOT IN (1,7) )
			OR (PC.PersonId IS NOT NULL AND PC.DayOff = 1)
			)
		AND (TEH.TimeEntryId IS NULL)
		AND C.Date >= @Today
		AND (C.Date <= @TerminationDate OR @TerminationDate IS NULL)
