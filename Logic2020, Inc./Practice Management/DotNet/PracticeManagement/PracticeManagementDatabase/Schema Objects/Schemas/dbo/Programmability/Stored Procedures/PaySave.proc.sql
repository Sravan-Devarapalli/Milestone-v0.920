...
AS
	...
	JOIN Calendar C ON (C.Date BETWEEN ISNULL(@PreviousRecordStartDate,CASE WHEN @StartDate > @OLD_StartDate  THEN @OLD_StartDate ELSE @StartDate END) AND 
		...
		AND TT.ChargeCodeDate = C.Date AND TT.ChargeCodeId  IN (@HolidayChargeCodeId ) AND TT.PersonId = @PersonId
	LEFT JOIN Pay P ON P.Person = TT.PersonId AND C.Date BETWEEN p.StartDate AND P.EndDate - 1
	...
	SELECT @PersonId
		,CASE WHEN PC.PersonId IS NOT NULL AND PC.DayOff <> C.DayOff AND ISNULL(PC.IsFloatingHoliday,0) = 0 THEN @PTOChargeCodeId ELSE @HolidayChargeCodeId END
		,C.Date
	...
	JOIN Person per ON per.PersonId = p.Person AND per.IsStrawman = 0
